...
def create_scheduling_function(scheduling_function_type : str,
                               num_epochs : int,
                               num_training_samples : int,):

    generate_probability = None
    steps_per_epoch = int(math.floor(num_training_samples / FLAGS.config.batch_size))
    total_steps = steps_per_epoch * num_epochs

    if scheduling_function_type == "constant":
        def generate_probability(step):
            prob = FLAGS.config.hybrid_config.p
            return prob
    
    elif scheduling_function_type == "piecewise1":
        piecewise_time =  FLAGS.config.hybrid_config.p * total_steps
        def generate_probability(step):
            if step <= piecewise_time:
                return 0.
            else:
                return 1.
    elif scheduling_function_type == "piecewise2":
        piecewise_time =  FLAGS.config.hybrid_config.p * total_steps
        def generate_probability(step):
            if step <= piecewise_time:
                return 1.
                ...
                return 0.
    elif scheduling_function_type == "piecewise3":
        piecewise_time =  0.5 * total_steps
        def generate_probability(step):
            if step <= piecewise_time:
                return FLAGS.config.hybrid_config.p
            else:
                return 1 - FLAGS.config.hybrid_config.p
    elif scheduling_function_type == "linear1":
        y_p =  FLAGS.config.hybrid_config.p
        if y_p <= 0.5:
            bl = 0
            al = 2 * y_p / total_steps
        else:
            bl = 2 * y_p - 1
            al = 2 * (1 - y_p) / total_steps
        def generate_probability(step): 
            return al * step + bl
    elif scheduling_function_type == "linear2":
        bl = FLAGS.config.hybrid_config.p
        al = 2 * (0.5 - bl) / total_steps
        def generate_probability(step): 
            return al * step + bl        
    else:
        raise ValueError("Unknown Scheduling Function Type!!")
